...
bool TextIter::GoNextChar()
{
  ...
  if (node->GetLevel() == level::LETTER && node->GetNext() != nullptr)
  {
    node = node->GetNext();
    return true;
  }
  ...
  if (node->GetLevel() == level::LETTER && node->GetNext() == nullptr)
  {
    if (st.IsEmpty())  throw -1; 

    TextNode* temp = st.Pop();
...
bool TextIter::IsEnd()
{
  ...
  if (node->GetLevel() == level::STRING || node->GetLevel() == level::WORD) return false;

  if (node->GetLevel() == level::LETTER && node->GetNext() != nullptr)return false;

  if (node->GetLevel() == level::LETTER && node->GetNext() == nullptr)
  {
    if (st.IsEmpty()) throw - 1; 

    TextNode* prev = st.Pop();
...
void TextIter::ResetToString() {
  while (node->GetLevel() > level::STRING) 
  {
    if (st.IsEmpty())throw - 1;
...
void TextIter::ResetToWord()
{
  ...
  else if (node->GetLevel() == level::LETTER)
  {
    node = st.Pop();
  }
}

TextNode* TextIter::Get() { return node;}

Text::Text(char* s) { root = new TextNode(s, level::STRING); }

Text::~Text() { TextNode::FreeMem(); }
...
TextIter Text::Find(char c)
{
  ...
  {
    if (iter.Get()->GetC() == c) return iter;
  }
  return iter;
}
...
TextIter Text::FindWord(char* c) 
{
  ...
  while (iter.GoNext()) 
  {
    if (iter.Get()->GetLevel() == level::WORD)
    {
...
void Text::FileLoad(string tmp)
{
    ...
    else
    {
        ...
        root = new TextNode(text, level::STRING);
...
inline void Text::DataInsert(char* k, TextIter i)
{
    ...
    else
    {
        ...
        if (next->GetC() == ' ' || next->GetC() == '.')
        {
            ...
            TextNode* node = new TextNode(z, lvl);
        ...
        else
        {
            ...
            TextNode* node = new TextNode(tmp, lvl);
            next->SetNext(node);
            i.Get()->SetNext(node);
        }
    }
...
void Text::Delete(int count, TextIter i)
{
    ...
    int symb_del = 0; 
    level lvl= i.Get()->GetLevel();

    if (lvl < level::LETTER)  i.GoNextChar(); 

    if (lvl>= level::STRING && lvl<= level::LETTER)
    {
        stack[static_cast<int>(lvl) - 1].Push(i.Get()); 
        if (lvl== level::LETTER)
        {
            symb_del++;
        }
    ...
    {
        lvl= iter.Get()->GetLevel(); 

        if (lvl >= level::STRING && lvl <= level::LETTER)
        {
            stack[static_cast<int>(lvl) - 1].Push(iter.Get()); 
    ...
    if (stack[0].IsEmpty() && stack[1].IsEmpty())
    {
        ...
        TextNode* last = stack[2].Pop();
        if (prev.Get()->GetLevel() == level::WORD)
        {
    ...
    if (stack[0].IsEmpty()) 
    {
        TextNode* last_word = stack[1].Pop();
        TextNode* last_letter = stack[2].Pop();
        TextIter cur = i;
        cur.ResetToWord();
    ...
    {
        TextNode* last_string = stack[0].Pop();
        TextNode* last_word = stack[1].Pop();
        TextNode* last_letter = stack[2].Pop();
        TextIter cur = i;
        cur.ResetToString();
...
char* Text::Copy(int count, TextIter i)
{
  ...
  while (iter.Get()->GetLevel() < level::LETTER)
  {
    iter.GoNext();
  }
